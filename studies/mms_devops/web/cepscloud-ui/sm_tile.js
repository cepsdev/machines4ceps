...
function setup_tile(tile_scale,
    ...
    if (minimized){
        $(`#sm_viz_${sm_name}`).animate({height:`${layout_info.sm_minimized.outter_height}px`},500,"",
            function(){
                $(`#sm_viz_details_svg_div_${sm_name}`).html("");
                $(`#sm_viz_details_${sm_name}`).css("height",0);
                $(`#info_${sm_name}`).removeClass("sm-details-info");
                $(`#info_${sm_name}`).html("");        
            });
        return;
    }

    d3_util_scale(d3.select(`#sm_viz_details_${sm_name}`).select("svg").select("g"),tile_scale,tile_scale);
    $(`#sm_viz_${sm_name}`).css("height",0);

    $(`#sm_viz_details_${sm_name}`).css("height",layout_info.sm_widget_tile.height);
    
    $(`#sm_viz_${sm_name}`).animate({height:`${layout_info.sm_widget_tile.outter_height}px`},500);
    $(`#sm_viz_${sm_name}`).css("height",layout_info.sm_widget_tile.outter_height);
    $(`#sm_viz_details_${sm_name}`).css("overflow","hidden");
    $(`#sm_viz_${sm_name}`).css("overflow","hidden");


    $(`#sm_viz_details_${sm_name}`).mouseenter(
        function(ev){
          //if (!$(`#sm_viz_${sm_name}`).hasClass("ceps-ui-tile")) return;
            if (tile_scale < 1.0) {
                new_tile_scale = tile_scale * 5.0;
                if (new_tile_scale > 1.0) new_tile_scale = 1.0;
                let svg_containing_div = $(`#sm_viz_details_svg_div_${sm_name}`);
                d3_util_scale(d3.select(`#sm_viz_details_svg_div_${sm_name}`).select("svg").select("g"),new_tile_scale,new_tile_scale);
            }                                        
        }
    );

    $(`#sm_viz_details_${sm_name}`).mouseleave(
        function(ev){
            //if (!$(`#sm_viz_${sm_name}`).hasClass("ceps-ui-tile")) return;
            let sm_viz_container = $(`#sm_viz_details_svg_div_${sm_name}`);
            d3_util_scale(d3.select(`#sm_viz_details_svg_div_${sm_name}`).select("svg").select("g"),tile_scale,tile_scale);
    ...
    $(`#sm_viz_details_${sm_name}`).mousemove(
        function(ev){
            //if (!$(`#sm_viz_${sm_name}`).hasClass("ceps-ui-tile")) return;
            let sm_viz_container = $(`#sm_viz_details_svg_div_${sm_name}`);
            ...
            let w_parent = $(`#sm_viz_details_${sm_name}`).width();
            let h_parent = $(`#sm_viz_details_${sm_name}`).height();

            let m_pos_y = ev.pageY - $(`#sm_viz_details_${sm_name}`).offset().top;
            let m_pos_x = ev.pageX - $(`#sm_viz_details_${sm_name}`).offset().left;
...
function handle_tile_click(sm_name){
    ...
    let sm_viz = $(`#sm_viz_${sm_name}`);$(sm_viz).removeClass("details-active");
    let sm_details = $(`#sm_viz_details_${sm_name}`);
    let svg_containing_div = $(`#sm_viz_details_svg_div_${sm_name}`);
    let svg_info = get_svg_infos(sm_name);
    let show_details_btn = $(`#show_details_btn_${sm_name}`);
    let sm_info = $(`#info_${sm_name}`);
    let add_svg = true;
    ...
    if (add_svg){
        ...
        //Scale to fit
        let svg_orig_height = parseFloat(d3.select(`#sm_viz_details_svg_div_${sm_name}`).select("svg").style("height"));
        let svg_orig_width = parseFloat(d3.select(`#sm_viz_details_svg_div_${sm_name}`).select("svg").style("width"));
