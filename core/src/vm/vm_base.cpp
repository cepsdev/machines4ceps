...
namespace ceps::vm::oblectamenta{
   ...
   size_t VMEnv::run(size_t start){
    ...
    size_t VMEnv::duptopi128(size_t pos){
        auto t{pop_cs<int64_t>()};
        auto tt{pop_cs<int64_t>()};
        push_cs(tt);push_cs(t);
        push_cs(tt);push_cs(t);
        return base_opcode_width +  pos;
    }
    size_t VMEnv::duptopi192(size_t pos){
        auto t{pop_cs<int64_t>()};
        auto tt{pop_cs<int64_t>()};
        auto ttt{pop_cs<int64_t>()};
        push_cs(ttt);push_cs(tt);push_cs(t);
        push_cs(ttt);push_cs(tt);push_cs(t);
        return base_opcode_width +  pos;
    }
    ...
    size_t VMEnv::asserti32imm(size_t pos){
    ...
    size_t VMEnv::asserteqi32sz(size_t pos){
        auto t1{pop_cs<int32_t>()}; auto t2{pop_cs<int32_t>()};
        if (t1 != t2) {
            auto msg_sz_loc = *((addr_t*)(text+pos+base_opcode_width));
            string msg {(char*)(mem.base+msg_sz_loc)};
            throw std::runtime_error{msg+"[asserteqi32sz values are not equal]." };
        }
        return base_opcode_width +  sizeof(addr_t) + pos;
    }

    size_t VMEnv::asserti32immsz(size_t pos){
        auto expected_value = *((int64_t*)(text+pos+base_opcode_width));
        auto t{pop_cs<int32_t>()};
        if (expected_value != (int64_t)t) {
            auto msg_sz_loc = *((addr_t*)(text+pos+base_opcode_width+sizeof(addr_t)));
            string msg {(char*)(mem.base+msg_sz_loc)};
            throw std::runtime_error{msg+"[asserti64(immsz) Expected " + std::to_string(expected_value) + " read " + std::to_string(t) +"]." };
        }
    ...
    size_t VMEnv::dbg_printsz(size_t pos){
        auto msg =   (char*) (*(addr_t*)(text+pos+base_opcode_width) + mem.base);
        cout << msg;
        return base_opcode_width + sizeof(addr_t) + pos;
    }
    ...
    size_t VMEnv::assertsz(size_t pos){
        auto expected_sz_p =  *((addr_t*)(text+pos+base_opcode_width));
        auto sz_p = pop_cs<addr_t>();
        if (strcmp( (char*) (mem.base + expected_sz_p) , (char*) (mem.base +sz_p) )!=0 )
            throw std::runtime_error{"assertsz: Expected '" +string{(char*) (mem.base +expected_sz_p)} + "' read '" + string{(char*) (mem.base +sz_p) } +"'." };
        return base_opcode_width + sizeof(addr_t) + pos;
    }
    ...
    size_t VMEnv::dbg_println(size_t pos){cout  << '\n'; return base_opcode_width + pos;} 
    ...
    size_t VMEnv::lddblimm(size_t pos){
        push_cs(*((double*)(text+pos+base_opcode_width))) ;
        return base_opcode_width + sizeof(double) + pos;
    }
...
// Replace address on stack with referenced value
    ...
    size_t VMEnv::beqi8(size_t pos){
        if (pop_cs<int8_t>() == pop_cs<int8_t>()) return *(size_t*)(text + pos  + base_opcode_width);
        return base_opcode_width + sizeof(addr_t) + pos;
    }
    size_t VMEnv::beqi64(size_t pos){
        if (pop_cs<int64_t>() == pop_cs<int64_t>()) return *(size_t*)(text + pos  + base_opcode_width);
        return base_opcode_width + sizeof(addr_t) + pos;
    }
    size_t VMEnv::bneqi8(size_t pos){
        if (pop_cs<int8_t>() != pop_cs<int8_t>()) return *(size_t*)(text + pos  + base_opcode_width);
        return base_opcode_width + sizeof(addr_t) + pos;
    }
    size_t VMEnv::bneqi64(size_t pos){
        if (pop_cs<int64_t>() != pop_cs<int64_t>()) return *(size_t*)(text + pos  + base_opcode_width);
        return base_opcode_width + sizeof(addr_t) + pos;
    }
    ...
    size_t VMEnv::bzeroi8(size_t pos){
        if (pop_cs<int8_t>() == 0) return *(size_t*)(text + pos  + base_opcode_width);
        return base_opcode_width + sizeof(addr_t) + pos;
    }
    size_t VMEnv::bnzeroi8(size_t pos){
        if (pop_cs<int8_t>() == 0) return *(size_t*)(text + pos  + base_opcode_width);
        return base_opcode_width + sizeof(addr_t) + pos;
    }
    ...
    size_t VMEnv::swpi8i64(size_t pos){
    ...
    size_t VMEnv::swpi8i128(size_t pos){
        auto t1{pop_cs<int8_t>()};
        auto t2{pop_cs<int64_t>()};
    ...
    size_t VMEnv::swp128i64(size_t pos){
        auto t1{pop_cs<int64_t>()};
        auto t2{pop_cs<int64_t>()};
        auto t3{pop_cs<int64_t>()};
    ...
    size_t VMEnv::swp192i64(size_t pos){
        auto t1{pop_cs<int64_t>()};
        auto t2{pop_cs<int64_t>()};
        auto t3{pop_cs<int64_t>()};
    ...
    size_t VMEnv::swp96i64(size_t pos){
        auto t1{pop_cs<int64_t>()};
        auto t2{pop_cs<int32_t>()};
        auto t3{pop_cs<int64_t>()};
        push_cs(t2);push_cs(t1);push_cs(t3);
        return base_opcode_width + pos;
    }
    ...
    size_t VMEnv::swp72i64(size_t pos){
        auto t1{pop_cs<int64_t>()};
    ...
    size_t VMEnv::swpi64b72(size_t pos){
        auto t1{pop_cs<int64_t>()};
        auto t2{pop_cs<int64_t>()};
        auto t3{pop_cs<int8_t>()};
        push_cs(t1);push_cs(t3);push_cs(t2);
        return base_opcode_width + pos;
    }    

    size_t VMEnv::swp128b8(size_t pos){
        auto t1{pop_cs<int64_t>()};
        auto t2{pop_cs<int64_t>()};
        auto t3{pop_cs<int8_t>()};
        push_cs(t2);push_cs(t1);push_cs(t3);
    ...
    size_t VMEnv::swpi64i192(size_t pos){
        auto t1{pop_cs<int64_t>()};
        auto t2{pop_cs<int64_t>()};
        auto t3{pop_cs<int64_t>()};
        auto t4{pop_cs<int64_t>()};
        push_cs(t1);push_cs(t4);push_cs(t3);push_cs(t2);
        return base_opcode_width + pos;
    }    
    ...
    VMEnv::VMEnv(){
        ...
        op_dispatch.push_back(&VMEnv::swpi32i64);
        op_dispatch.push_back(&VMEnv::swp128i64);
        op_dispatch.push_back(&VMEnv::swp80i64);
        op_dispatch.push_back(&VMEnv::swpi64b72);
        op_dispatch.push_back(&VMEnv::swp72i64);
        op_dispatch.push_back(&VMEnv::swp128b8);
        op_dispatch.push_back(&VMEnv::lddblimm);
        op_dispatch.push_back(&VMEnv::duptopi128);
        op_dispatch.push_back(&VMEnv::swpi8i64);
        op_dispatch.push_back(&VMEnv::beqi8);
        ...
        op_dispatch.push_back(&VMEnv::dbg_println);
    }     
