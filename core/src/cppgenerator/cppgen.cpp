...
void write_copyright_and_timestamp(std::ostream& out, std::string title,bool b,Result_process_cmd_line const& result_cmd_line){
	...
	out
		...
		<< "   GENERATED BY THE sm4ceps C++ GENERATOR VERSION 0.75 (c) 2016 Tomas Prerovsky <tomas.prerovsky@gmail.com>, ALL RIGHTS RESERVED. \n"
    ...
    if (all_xml_frames.size()){
    	for(auto const & e : all_xml_frames.nodes() ){
    		...
    		if (body.size())
    			...
    			cppgenerator.xml_in_frames()[id].ceps_query = body;
