...
ctrlpanel{${ctrlPanelEditor.get_name() ? 'name{"' + ctrlPanelEditor.get_name() + '";};' : ''}`+
    (() => {
        ...
        for (let ch of $("#" + ctrl_editor_id).children()) {
            ...
            function get_type() { if (winf.type.sig) { if (winf.toggle_sig) return "toggle_signal"; if (winf.watch_only) return "watch_signal"; if (winf.do_plot) return "plot_signal"; } return undefined; }
 ...
 widget{
  type{${get_type()};};
  associated_systemstate{as_symbol("${get_assoc_sysstate()}","Systemstate");};
  style{${get_style()};};    
 };` 
               ;
            }
